...
{{if $tn}}
    ...
    {{if and .ReactionAdded (or (eq .User.ID $creator) $isMod $isAdmin) (eq $RID (toInt $master.mainMsgID)) (ne (toInt $master.pos) 3)}}
        ...
        {{else if eq .Reaction.Emoji.Name $cce}}
            ...
            {{$ced := print "Ce ticket a été fermé par : " .User.Mention}}
            {{$ce := cembed "color" 16776960 "description" $ced "timestamp" currentTime}}
            {{sendMessageNoEscape nil $ce}}
            {{$ced2 := print $ste " Enregistrer la transcription\n" $roe " Rouvrir le ticket\n" $de " Supprimer le ticket"}}
    ...
    {{if and (or $isMod $isAdmin) (eq $RID (toInt $master.mainMsgID)) (eq .Reaction.Emoji.Name $se)}}
        {{if .ReactionAdded}}
            {{if ne (toInt $master.pos) 2}}
                ...
                {{$desc := print "Maintenant " .User.Mention " résout ce ticket !"}}
        ...
        {{else}}
            {{if ne (toInt $master.pos) 1}}
                ...
                {{$desc := print "On dirait que " .User.Mention " n'a pas pu résoudre votre problème.\nNe t'inquiète pas ! Un autre membre du personnel vient vous aider dès que possible !"}}
                {{$emb := cembed "description" $desc "color" 1752220}}
                {{sendMessage nil $emb}}
                {{$master.Set "pos" 1}}
                {{$master.Set "resp" 0}}
    ...
    {{if and $isAdmin (eq $RID (toInt $master.mainMsgID)) (eq .Reaction.Emoji.Name $aoe)}}
        {{if .ReactionAdded}}
            {{$desc := print "Ce ticket est maintenant en mode administrateur uniquement"}}
            {{$emb := cembed "description" $desc "color" 1752220}}
            {{sendMessage nil $emb}}
            {{$s := exec "ticket ao"}}
        {{else}}
            {{$desc := print "Ce ticket n'est plus en mode administrateur uniquement"}}
            {{$emb := cembed "description" $desc "color" 1752220}}
            {{sendMessage nil $emb}}
            {{$s := exec "ticket ao"}}
        {{end}}
    ...
    {{if and (or $isMod $isAdmin) (eq (toInt $master.lastMessageID) $RID) .ReactionAdded}}
        {{if eq .Reaction.Emoji.Name $de}}
            {{if ne (toInt $master.rodando) 1}}
                {{sendMessage nil (cembed "description" "Ce ticket sera bientôt supprimé." "color" 15158332)}}
        ...
        {{else if eq .Reaction.Emoji.Name $roe}}
            ...
            {{sendMessage nil (cembed "description" (print "Ce billet a été rouvert par : " .User.Mention) "color" 255)}}
        ...
        {{else if eq .Reaction.Emoji.Name $ste}}
            {{sendMessage $Trc (cembed "title" (printf "Ticket %s transcription" $tn) "description" (execAdmin "logs 250"))}}
            {{sendMessage nil (cembed "description" "Transcription enregistrée !" "color" 3066993)}}
