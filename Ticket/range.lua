...
{{if .ExecData.um}}
    {{if ge $counter 1}}
        {{if le (len $entries) 5}}
            ...
            {{with .ExecData.msgID}} {{editMessage nil . (cembed "description" "Tous les utilisateurs ont été supprimés de ce ticket." "color" 1146986)}}
            {{else}} {{sendMessage nil (cembed "description" "Tous les utilisateurs ont été supprimés de ce ticket." "color" 1146986)}}
            {{end}}
        {{else}}
            {{$msgID := sendMessageRetID nil (cembed "description" "Plus de 5 utilisateurs étaient sur ce ticket. Attendez de pouvoir effectuer une action !" "color" 15105570)}}
            {{deleteResponse 5}}
...
{{else if .ExecData.dois}}
    {{if ge $counter 1}}
        {{if le (len $entries) 5}}
            ...
            {{$master.Set "rodando" 0}}
            {{dbSet (toInt $tn) "ticket" $master}}
            {{with .ExecData.msgID}} {{editMessage nil . (cembed "description" "Tous les utilisateurs ont été de nouveau ajoutés à ce ticket." "color" 1146986)}}
            {{else}} {{sendMessage nil (cembed "description" "Tous les utilisateurs ont été de nouveau ajoutés à ce ticket." "color" 1146986)}}
            {{end}}
        {{else}}
            {{$msgID := sendMessageRetID nil (cembed "description" "Plus de 5 utilisateurs étaient sur ce ticket. Ajout d'utilisateurs." "color" 15105570)}}
            {{deleteResponse 5}}
            {{$entries = dbTopEntries $key 5 0}}
            {{range $entries}} {{- $s := execAdmin "ticket adduser" .User.ID -}} {{end}}
            {{$master.Set "rodando" 1}}
...
{{else if .ExecData.tres}}
    {{if ge $counter 1}}
        {{if le (len $entries) 5}}
            ...
            {{with .ExecData.msgID}}
                {{editMessage nil . (cembed "description" "Tous les utilisateurs ont été supprimés. Le ticket est en cours de suppression." "color" 1146986)}}
            {{else}}
                {{sendMessage nil (cembed "description" "Tous les utilisateurs ont été supprimés. Le ticket est en cours de suppression." "color" 1146986)}}
            {{end}}
            {{$s := execAdmin "ticket close" ""}}
        {{else}}
            {{$msgID := sendMessageRetID nil (cembed "description" "Plus de 5 utilisateurs étaient sur ce ticket. Je les supprime." "color" 15105570)}}
            {{deleteResponse 5}}
        ...
        {{end}}
    {{else}}
        {{sendMessage nil (cembed "description" "Le ticket est en cours de suppression." "color" 1146986)}}
        {{dbDel (toInt $tn) "ticket"}}
        {{$s := execAdmin "ticket close" ""}}
        {{$map := sdict}}
...
{{editMessage .TemplateArgs.Master .TemplateArgs.MsgID (cembed "title" "Affichage des billets" "color" (randInt 16777216) "description" $desc)}}
{{end}}