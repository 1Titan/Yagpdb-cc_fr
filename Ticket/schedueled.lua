    ...
    {{if $tn}}
        ...
        {{if not (eq (toInt $master.pos) 3)}}
            {{if (eq $alert 1)}}
                ...
                {{$embed := cembed "description" "Ce billet sera fermé dans 3 heures pour cause d'inactivité !" "Title" "ATTENTION !"}}
                {{$content := print "Le ticket " $tn " sera bientôt fermé !\n\n" mentionEveryone}}
            ...
            {{else if eq $alert 2}}
                ...
                {{$master.Set "alert" 2}}
                {{dbSet (toInt $tn) "ticket" $master}}
                {{$embed := cembed "description" "Ce billet sera fermé dans 30 minutes en raison de l'inactivité!" "title" "ATTENTION !"}}
                {{$content := print "Le ticket " $tn " est sur le point d'être fermé !\n\n" mentionEveryone}}
                {{sendMessageNoEscape nil (complexMessage "content" $content "embed" $embed)}}
            ...
            {{else if eq $alert 3}}
                ...
                {{$master.Set "alert" 3}}
                {{dbSet (toInt $tn) "ticket" $master}}
                {{sendMessage nil (cembed "description" "Ce ticket est fermé en raison de son inactivité!" "color" 15158332)}}
