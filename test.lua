{{/* Satff Roles */}}
        {{$Admins := cslice 766987293277814785}} {{/* IDs of your ADMINs Roles. Leave the "cslice" here even if you have only 1 role */}}
        {{$Mods := cslice 766987296079478805}} {{/* IDs of your MODs Roles. Leave the "cslice" here even if you have only 1 role */}}
        {{$MentionRoleID := 766987307001315328}} {{/* Role to be mentioned when a new ticket is opened */}}

    {{/* Open Message Info */}}
        {{$msgOpenChannelID := 780644447707988028}} {{/* Channel ID where the msg to open tickets is at. THIS CHANNEL CANT BE IN THE SAME CATEGORY AS THE TICKETS!!!!! */}}
        {{$msgID := 780644457548480536}} {{/* Message ID of the message the user has to react to open ticket */}}
