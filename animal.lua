{{/*Trigger : Command `animal`*/}}

{{ $args := parseArgs 1 "Usage : `-animal <nom animal>`. Usage `-animal help` pour voir les types d'animaux disponibles." 
(carg "string" "animal")
...
{{ $send = false }}
{{ else }}
Usage : `-animal <nom animal>`. Usage `-animal help` pour voir les types d'animaux disponibles.
{{ $send = false }}
{{ end }}
{{ if $send }}
